...
Unity Test API
==============
Each test file main function  calls UNITY_BEGIN(Module name,date,time), then RUN_TEST for each test, and finally UNITY_END().

Running Tests
-------------
UNITY_BEGIN(Module name,date,time)
    It displays Module name along with  compilation time and date. Clears Statistics of the unity structures.
...
UNITY_END()
    Returns test Statistics (which is the total number of Tests, number of  failures and number of Ignored Test cases).
============================================================================ */
...
static const char UnityStrPass[]                   = "PASSED";
static const char UnityStrFail[]                   = "[FAILED]";
static const char UnityStrIgnore[]                 = "[IGNORED]";
#endif
...
static void UnityTestResultsForPassedCondition(const char* file, const UNITY_LINE_TYPE line)
{
       UnityPrint("TEST ");
       UnityPrintNumber((UNITY_INT)(Unity.NumberOfTests));
       UNITY_OUTPUT_CHAR(':');
...
void UnityConcludeTest(void)
{
    ...
    else if (!Unity.CurrentTestFailed)
    {
        ...
        UnityTestResultsForPassedCondition(Unity.TestFile, Unity.CurrentTestLineNumber);
...
/*-----------------------------------------------*/
void UnityBegin(const char* filename, const char*testName,const char*date,const char*time)
{
    /* Initialization of UART */
    #ifndef DISABLE_UNITY_UART_INIT
    uart_init();
    #endif

    Unity.TestFile = filename;
    Unity.testName = testName;
...
/* UART Device number to use for this example */
#define UART_DEVICE_NUM         ADI_UART_DEVICE_0
/* UART Driver control block to use */
#define UartDrv                 Driver_USART0
/* Echo every # characters */
#define UART_BUF_RX_SIZE           1u


/* Check adi_uart_config.h for baud rate configuration */

/* UART Tx Buffer */
static char  nBufferTx[UART_BUF_RX_SIZE] = { 0 };
/* UART Tx Buffer */
static char  nBufferRx[UART_BUF_RX_SIZE] = { 0 };
...
extern void init_cli_handler(void);

/* Flags to monitor data transfer */
volatile bool RxInProgress =true;
...
#ifndef RETARGET_ITM
/*Retaget printf  to UartDrv.Send Funtion*/
int stdout_putchar(int c, FILE * stream)
{
    TxInProgress = true;
	UartDrv.Send(&c,1);
    while (TxInProgress) { ; }
    /*return the character written to denote a successfull write*/
	return c; 
}
#endif

/* UART callback */
void UART_Callback(uint32_t nEvent)
{
    /* IF (UART Tx complete) */
    if (nEvent == ARM_USART_EVENT_SEND_COMPLETE)
    {
        TxInProgress = false;
    }

    /* IF (UART Rx complete) */
    if (nEvent == ARM_USART_EVENT_RECEIVE_COMPLETE)
    {
        UartDrv.Receive(nBufferRx, UART_BUF_RX_SIZE);
        RxInProgress = false;
    }
}
...
void uart_init(void)
{
    ...
    UartResult = UartDrv.Initialize (UART_Callback);
    
    if (UartResult == ARM_DRIVER_OK)
    {
        /* Power up UART device */
        UartResult = UartDrv.PowerControl(ARM_POWER_FULL);
    }
 
    /*UART Rx buffer submission*/
    UartDrv.Receive(nBufferRx, UART_BUF_RX_SIZE);  
}
