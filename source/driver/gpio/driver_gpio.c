/********************************************************************************
* @file     driver_gpio.c                                                       *
* @brief    File contains implementations of GPIO driver functions.             *
...
common_rc_t driver_gpio_reset (GPIO_TypeDef* pGpioInst)
{
    ...
    /* Reset the peripheral by setting and clearing 
       the reset bit for the respective GPIO instance */
    if (pGpioInst == GPIOA) 
    {
...
void driver_gpio_write_pin (GPIO_TypeDef* pGpioInst, bool bSet, uint8_t pinNumber)
{
    if (bSet) 
    {
        /* Set the GPIO port pin in BSRR */
        pGpioInst->BSRR |= (uint32_t)(SET << pinNumber);
    }
    else 
    {
